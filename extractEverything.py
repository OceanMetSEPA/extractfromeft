...
def prepareDir(outputDirP):
  if outputDirP is None:
    ...
    # output files.
    outputDir = os.getcwd()
    logfilename = path.join(outputDir, 'extractEverything_{}.log'.format(datetime.now().strftime('%Y%m%d%H%M%S')))
    if not path.isdir(path.join(outputDir, 'temp')):
      os.makedirs(path.join(outputDir, 'temp'))
  else:
    outputDir = outputDirP
    logfilename = path.join(outputDir, 'extractEverything.log')
...
def main():
  ...
  # Create the log file.
  logger = logging.getLogger('extractEverything')
  ...
  with open(logfilename, 'r') as logf:
    for line in logf:
      ...
      if (CompletedSearchStr in line) or (SkippedSearchStr in line):
        ci += 1
        if CompletedSearchStr in line:
          info = line[line.find(CompletedSearchStr)+len(CompletedSearchStr):-2]
        else:
          info = line[line.find(SkippedSearchStr)+len(SkippedSearchStr):-2]
        infosplt = info.split(',')
        completed.loc[ci] = [infosplt[0].strip(), int(infosplt[1].strip()), int(infosplt[2].strip()), infosplt[3].strip(), infosplt[4].strip()]

  processEFT(pargs.inputfile, pargs.outputdir, pargs.area, pargs.years, euroClasses=pargs.euros, splitSize=True, splitBusCoach=True, splitEuroTech=True, keepTempFiles=pargs.keeptemp, completed=completed)
